...
import { validateParams } from './middleware/validateIngredient';
...
*/
router.post("/api/ingredients", authenticate, validateParams, async (req: Request, res: Response) => {
    try {
...
If the resource exists but it isn't owned by the requesting user, it will throw a 403.
*/
router.patch("/api/ingredients/:id", authenticate, validateParams, async (req: Request, res: Response) => {
    const userId: string = req.body.user_id;
